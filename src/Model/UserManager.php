...
class UserManager
{
    ...
    public function create(string $login, string $password, bool $isAdmin = false, bool $isActive = false, string $birthdate = null): int
    {
        $hash = $this->hashPassword($password);
        $statement = $this->database->prepare('INSERT INTO users (login, password, is_admin, is_active, birthdate) VALUES (:login, :password, :is_admin, :is_active, :birthdate)');
        ...
        $statement->bindParam(':birthdate', $birthdate, Database::PARAM_STR);
        $statement->execute();
